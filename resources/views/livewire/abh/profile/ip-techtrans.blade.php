    ...
    <div class="space-y-4">
        <div class="border border-gray-300 dark:border-gray-700 rounded p-2 flex justify-between items-center">
            ...
            <x-pop-modal name="EditPolicy" class="max-w-md" static="true" modal-title="IP Policy Update">
                <form wire:submit.prevent="saveIpPolicy" class="space-y-5">
                    <ul class="grid w-full gap-6 md:grid-cols-2">
                        <li>
                            <input wire:model="ipPolicy" value="1" type="radio" id="hosting-small" name="hosting"
                        ...
                        <li>
                            <input wire:model="ipPolicy" value="0" type="radio" id="hosting-big" name="hosting"
                                   class="hidden peer">
                    ...
                    <x-input-error :messages="$errors->get('ipPolicy')"/>
                    <x-alert-success :message="session('ipPolicy')"/>
                    <x-submit-button wire:loading.attr="disabled" wire:target="saveIpPolicy" class="min-w-full">
                        <div wire:loading wire:target="saveIpPolicy" class="w-full text-center p-2">
                            Processing...
                        </div>
                        <div  wire:loading.remove wire:target="saveIpPolicy" class="w-full text-center p-2">
                            Save
                        </div>
                    </x-submit-button>
                </form>
        ...
        <div class="border border-gray-300 dark:border-gray-700 rounded p-2 flex justify-between items-center">
            ...
            <x-pop-modal name="EditTechTrans" class="max-w-md" static="true" modal-title="Tech Trans Protocol">
                <form wire:submit.prevent="saveTechTrans" class="space-y-5">
                    <ul class="grid w-full gap-6 md:grid-cols-2">
                        <li>
                            <input wire:model="techTrans" value="1" type="radio" id="hosting-small2" name="host"
                                   class="hidden peer" required>
                        ...
                        <li>
                            <input wire:model="techTrans" value="0" type="radio" id="hosting-big2" name="host"
                                   class="hidden peer">
                    ...
                    <x-input-error :messages="$errors->get('techTrans')"/>
                    <x-alert-success :message="session('techTrans')"/>
