<div class="border rounded shadow-lg border-gray-300 dark:border-gray-600">
    ...
    <div class="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 p-2">
        ...
        <x-pop-modal name="updateMod-{{$univKey}}" modal-title="Update {{$title}} attachment" class="max-w-xl"
                     static="true">

            <form class="space-y-2" wire:submit.prevent="saveForm('{{$data}}')">
                <div>
                    <x-input-label value="Attachment"/>
                    <x-text-input wire:model="model" placeholder="upload pdf" type="file" accept="application/pdf"
                                  class="w-full"/>
        ...
        @if($fullTechDescription[$data])
            <div class="mt-4 flex justify-between items-center">
                ...
                <x-pop-modal name="pdfModFullTech-{{$univKey}}" class="max-w-7xl">
                    <iframe src="{{\Illuminate\Support\Facades\Storage::url($fullTechDescription[$data])}}"
                            class="aspect-video w-full"></iframe>
                </x-pop-modal>
                <x-secondary-button class="text-sky-600 dark:text-sky-600"
                                    data-modal-toggle="pdfModFullTech-{{$univKey}}">
                    <div class="text-xs">
                        Open
                    </div>
                </x-secondary-button>
