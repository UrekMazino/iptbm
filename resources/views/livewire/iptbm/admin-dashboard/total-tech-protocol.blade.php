...
<div class="rounded-none md:rounded-lg  bg-gradient-to-tr from-yellow-800 to-yellow-300">
    <x-pop-modal name="crypto-modal-techProtocol" class="max-w-5xl max-h-screen" modal-title="Total IP-TBMs Technologies">
        <x-card>
            <div class="relative overflow-x-auto ">
                <table id="techProtocol" style="width:100%" class="w-fit display table-auto cell-border stripe table-auto  hover text-sm  rounded text-left text-gray-500  border-gray-300 dark:border-gray-600  dark:text-gray-400">
                    <thead class="text-base text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr class="border-0 ">
                        <th scope="col" class="region whitespace-nowrap px-10 py-3 border border-gray-300 dark:border-gray-600 text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            Region
                        </th>
                    ...
                    @foreach($profiles as $profile)
                        <tr>
                            <td>
                                <a href="{{route("iptbm.admin.editregion.index",['id'=>$profile->agency->region])}}" class="font-medium hover:text-gray-900 hover:dark:text-white hover:underline">
                                    {{$profile->agency->region->name}}
                                </a>

                            </td>
                            <td>
                                <a href="{{route("iptbm.admin.view-agency",['agency'=>$profile->agency])}}" class="font-medium hover:text-gray-900 hover:dark:text-white hover:underline">
                                    {{$profile->agency->name}}
                                </a>
    ...
    <div class="rounded-b-lg bg-yellow-800 bg-opacity-75 py-2 ">
        ...
        <div class="text-center w-full">
            <button data-modal-target="crypto-modal-techProtocol" data-modal-toggle="crypto-modal-techProtocol"  class="text-center text-gray-300 font-medium text-lg m-auto ">
                View List
