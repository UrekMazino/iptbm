...
@section('content')
    <div class="w-full">
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <a href="{{route("iptbm.staff.ip-management.application.index",['id'=>$ip_alert->id])}}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">Ip Application</a>
                            ...
                            <li aria-current="page">
                                <div class="flex items-center">
                                    ...
                                    <a href="{{route("iptbm.staff.ip-alert.task",['id'=>$ip_alert->id])}}" class="ml-1 text-sm font-medium hover:text-blue-600 md:ml-2 text-blue-700">Ip Task</a>
                                    <span class="ms-4 text-gray-400 dark:text-gray-600 text-sm hidden md:block">
                                        {{$ip_alert->ip_type->name}}
                                    </span>
                                </div>
                            </li>
                        </ol>
        ...
        @endif
        <div class="px-4 mt-4">
           <x-header-label class="mb-4 mt-7">
               {{$ip_alert->ip_type->name}}  Task
           </x-header-label>
            @foreach($tasks as $key=>$task)

                <livewire:iptbm.staff.ip-management.iptask.ip-task wire:key="{{$task->id}}" :task="$task"  :ipAlert="$ip_alert" />
